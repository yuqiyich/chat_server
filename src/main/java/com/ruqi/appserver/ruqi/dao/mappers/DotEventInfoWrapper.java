...
public interface DotEventInfoWrapper {
    ...
    @Insert("insert into dot_event_record(event_key,user_id,device_id,event_detail,create_time,record_time,device_brand," +
            ...
            "#{dotEventInfo.content.eventDetail},FROM_UNIXTIME(#{dotEventInfo.content.createTime} / 1000,'%Y-%m-%d %H:%i:%s'),DATE_FORMAT(NOW(),'%Y-%m-%d %H:%i:%s')," +
    ...
    @Select({"<script>",
            ...
            "<if test='dotEventInfo.content.orderIdFilter==\"3\" '>GROUP BY order_id </if>",
            " </if>",
    ...
    @Select({"<script>",
            "<if test='dotEventInfo.content.orderIdFilter!=\"3\" '>SELECT count(*) FROM dot_event_record </if>",
            "<if test='dotEventInfo.content.orderIdFilter==\"3\" '>SELECT count(DISTINCT(order_id)) FROM dot_event_record </if>",
            "WHERE 1=1",
            ...
            "</if>",
            "</script>"})
    long queryCommonEventListSizeForLayui(RecordInfo<DotEventInfo> dotEventInfo, String eventKey);
    ...
    @Select({"<script>",
            "SELECT AVG(SUBSTRING(event_detail, 19, LENGTH(event_detail) - 19)) FROM dot_event_record ",
            "WHERE 1=1 AND event_key = 'FB_SYNC_TX_RESUME' AND LENGTH(event_detail) > 0 ",
            "<if test='dotEventInfo.content!=null '>",
            "<if test='dotEventInfo.content.appId!=null and dotEventInfo.content.appId!=\"\" and dotEventInfo.content.appId>0'>AND app_id=#{dotEventInfo.content.appId}  </if>",
            "<if test='dotEventInfo.content.platform!=null and dotEventInfo.content.platform!=\"\" '>AND platform = #{dotEventInfo.content.platform} </if>",
            "<if test='dotEventInfo.content.startDate!=null '>AND record_time &gt; #{dotEventInfo.content.startDate}  </if>",
            ...
            "</if>",
            "</script>"})
    String querySyncTecentAvgTime(@Param("dotEventInfo") RecordInfo<DotEventInfo> dotEventInfo);

    @Select({"<script>",
            "select avg(distinct time) from ( ",
            "select t1.time from ",
            "(SELECT SUBSTRING(event_detail, 19, LENGTH(event_detail) - 19) as time FROM dot_event_record ",
            "WHERE 1=1 AND event_key = 'FB_SYNC_TX_RESUME' AND LENGTH(event_detail) > 0 ",
            "<if test='dotEventInfo.content!=null '>",
            "<if test='dotEventInfo.content.appId!=null and dotEventInfo.content.appId!=\"\" and dotEventInfo.content.appId>0'>AND app_id=#{dotEventInfo.content.appId}  </if>",
            "<if test='dotEventInfo.content.platform!=null and dotEventInfo.content.platform!=\"\" '>AND platform = #{dotEventInfo.content.platform} </if>",
            ...
            ") as t2 ",
            "group by t1.time ",
            "having SUM(case when t2.time &gt;= t1.time then 1 else 0 end) &gt;= (count(*) / 2.0) ",
            "AND SUM(case when t2.time &lt;= t1.time then 1 else 0 end) &gt;= (count(*) / 2.0) ",
            ") as tmp",
            "</script>"})
    String querySyncTecentMedianTime(@Param("dotEventInfo") RecordInfo<DotEventInfo> dotEventInfo);
    ...
    @Select({"<script>",
            ...
            "(SELECT COUNT(*) as tx_geo_total_count_android FROM dot_event_record WHERE ",
            "record_time &gt; #{startTime} AND record_time &lt; #{endTime}",
            "AND app_id=#{appId} AND event_key='FALLBACK_SUCCESS_TX_GEO' AND order_id is NULL ",
            "AND platform='Android') as tgta,",
            "(SELECT COUNT(*) as tx_geo_order_count_android FROM dot_event_record WHERE ",
            "record_time &gt; #{startTime} AND record_time &lt; #{endTime}",
            "AND app_id=#{appId} AND event_key='FALLBACK_SUCCESS_TX_GEO' AND order_id is NOT NULL ",
            "AND platform='Android') as tgoa,",
            "(SELECT COUNT(*) as tx_geo_total_count_ios FROM dot_event_record WHERE ",
            "record_time &gt; #{startTime} AND record_time &lt; #{endTime}",
            "AND app_id=#{appId} AND event_key='FALLBACK_SUCCESS_TX_GEO' AND order_id is NULL ",
            "AND platform='iOS') as tgti,",
            "(SELECT COUNT(*) as tx_geo_order_count_ios FROM dot_event_record WHERE ",
            "record_time &gt; #{startTime} AND record_time &lt; #{endTime}",
            "AND app_id=#{appId} AND event_key='FALLBACK_SUCCESS_TX_GEO' AND order_id is NOT NULL ",
            "AND platform='iOS') as tgoi,",

            "(SELECT COUNT(*) as app_total_count_android FROM dot_event_record WHERE ",
            "record_time &gt; #{startTime} AND record_time &lt; #{endTime}",
            "AND app_id=#{appId} AND event_key='FALLBACK_SUCCESS_RUQI_RECOMMEND' AND order_id is NULL ",
            "AND platform='Android') as ata,",
            "(SELECT COUNT(*) as app_order_count_android FROM dot_event_record WHERE ",
            "record_time &gt; #{startTime} AND record_time &lt; #{endTime}",
            "AND app_id=#{appId} AND event_key='FALLBACK_SUCCESS_RUQI_RECOMMEND' AND order_id is NOT NULL ",
            "AND platform='Android') as aoa,",
            "(SELECT COUNT(*) as app_total_count_ios FROM dot_event_record WHERE ",
            "record_time &gt; #{startTime} AND record_time &lt; #{endTime}",
            "AND app_id=#{appId} AND event_key='FALLBACK_SUCCESS_RUQI_RECOMMEND' AND order_id is NULL ",
            "AND platform='iOS') as ati,",
            "(SELECT COUNT(*) as app_order_count_ios FROM dot_event_record WHERE ",
            "record_time &gt; #{startTime} AND record_time &lt; #{endTime}",
            "AND app_id=#{appId} AND event_key='FALLBACK_SUCCESS_RUQI_RECOMMEND' AND order_id is NOT NULL ",
            "AND platform='iOS') as aoi,",

            "(SELECT COUNT(*) as app_geo_total_count_android FROM dot_event_record WHERE ",
            "record_time &gt; #{startTime} AND record_time &lt; #{endTime}",
            "AND app_id=#{appId} AND event_key='FALLBACK_SUCCESS_RUQI_GEO' AND order_id is NULL ",
            "AND platform='Android') as agta,",
            "(SELECT COUNT(*) as app_geo_order_count_android FROM dot_event_record WHERE ",
            "record_time &gt; #{startTime} AND record_time &lt; #{endTime}",
            "AND app_id=#{appId} AND event_key='FALLBACK_SUCCESS_RUQI_GEO' AND order_id is NOT NULL ",
            "AND platform='Android') as agoa,",
            "(SELECT COUNT(*) as app_geo_total_count_ios FROM dot_event_record WHERE ",
            "record_time &gt; #{startTime} AND record_time &lt; #{endTime}",
            "AND app_id=#{appId} AND event_key='FALLBACK_SUCCESS_RUQI_GEO' AND order_id is NULL ",
            "AND platform='iOS') as agti,",
            "(SELECT COUNT(*) as app_geo_order_count_ios FROM dot_event_record WHERE ",
            "record_time &gt; #{startTime} AND record_time &lt; #{endTime}",
            "AND app_id=#{appId} AND event_key='FALLBACK_SUCCESS_RUQI_GEO' AND order_id is NOT NULL ",
            "AND platform='iOS') as agoi,",

            "(SELECT COUNT(*) as fail_count_android FROM dot_event_record WHERE ",
            "record_time &gt; #{startTime} AND record_time &lt; #{endTime}",
            "AND app_id=#{appId} AND event_key='FALLBACK_FAIL_BOARDING_POINT' ",
            "AND platform='Android') as fta,",
            "(SELECT COUNT(*) as fail_count_ios FROM dot_event_record WHERE ",
            "record_time &gt; #{startTime} AND record_time &lt; #{endTime}",
            "AND app_id=#{appId} AND event_key='FALLBACK_FAIL_BOARDING_POINT' ",
            "AND platform='iOS') as fti",
            "</script>"})
    ...
    @Insert({"<script>",
            ...
            " tx_geo_total_count_android, tx_geo_order_count_android, tx_geo_total_count_ios, tx_geo_order_count_ios,",
            " app_total_count_android, app_order_count_android, app_total_count_ios, app_order_count_ios,",
            " app_geo_total_count_android, app_geo_order_count_android, app_geo_total_count_ios, app_geo_order_count_ios,",
            " fail_count_android, fail_count_ios",
            ...
            " #{dbEventDayDataRecPoint.tx_geo_total_count_android}, #{dbEventDayDataRecPoint.tx_geo_order_count_android}, #{dbEventDayDataRecPoint.tx_geo_total_count_ios}, #{dbEventDayDataRecPoint.tx_geo_order_count_ios},",
            " #{dbEventDayDataRecPoint.app_total_count_android}, #{dbEventDayDataRecPoint.app_order_count_android}, #{dbEventDayDataRecPoint.app_total_count_ios}, #{dbEventDayDataRecPoint.app_order_count_ios},",
            " #{dbEventDayDataRecPoint.app_geo_total_count_android}, #{dbEventDayDataRecPoint.app_geo_order_count_android}, #{dbEventDayDataRecPoint.app_geo_total_count_ios}, #{dbEventDayDataRecPoint.app_geo_order_count_ios},",
            " #{dbEventDayDataRecPoint.fail_count_android}, #{dbEventDayDataRecPoint.fail_count_ios}",
            " )",
            "</script>"})
